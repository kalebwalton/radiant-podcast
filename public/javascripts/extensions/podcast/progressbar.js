...
ProgressBar.prototype =  {
   ...
   initialize: function( container, options ) {
	...
	_updateSelection: function(){
		if( ( this.options.style & ProgressBar.INDETERMINATE ) == ProgressBar.INDETERMINATE ){
			...
			while( this.progressIndicators.length < nbIndicators ){
				...
				var element = this._createIndicator( ( i * ( this.options.widthIndicators ) ) + 'px' ); // KW - Removed the + 1 so there's no spacing 
	...
	_createIndicator: function( left ){
      ...
      var height = this.progressBarContent.clientHeight+1;
			var filtersParams = [
				['absolute', '0px', null, '100%', height + 'px', bgColor, element, 1]
			];
    ...
    _drawDhtmlElements: function( ){
		...
		this.progressBarContent = this._createElement(['absolute', '2px', '3px', (size.width - 6) + 'px', (size.height - 5) + 'px', null, this.progressBar]);
